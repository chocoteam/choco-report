...
public class BugsComponent extends CustomComponent {
    ...
    // 3 bug kinds
    final int[] bugs = new int[]{0, 1, 2, 3};
    public static final int NO_SOL = 0, WRONG_OPT = 2, SUSPICIOUS_VALUE = 1, NO_NODE=3;
    // data
    int[] nbBugs = new int[4];
    List<String>[][] prob_name = new List[2][4];
    ...
    public BugsComponent(String benchmark1, String benchmark2, Properties mysql) {
        ...
        displayBugTable(table, NO_NODE, benchmark1, benchmark2, "ALL: 0 nodes in resolution");
    ...
    private void loadWrongOpt(Statement st, Integer BID1, Integer BID2) throws SQLException {
        ...
        // benchmark 1
        String query = "SELECT p.name FROM PROBLEMS p, RESOLUTIONS r WHERE bid = " + BID1 + " AND r.solving_time<900 AND r.pid=p.pid AND nb_sol<> 0 AND p.objective<>r.objective and p.resolution<>'SAT' ";
        ResultSet rs = st.executeQuery(query);
        while (rs.next()) {
            prob_name[0][WRONG_OPT].add(rs.getString("name"));
        }
        // benchmark 2
        query = "SELECT p.name FROM PROBLEMS p, RESOLUTIONS r WHERE bid = " + BID2 + " AND r.solving_time<900 AND r.pid=p.pid AND nb_sol<> 0 AND p.objective<>r.objective and p.resolution<>'SAT'";
        rs = st.executeQuery(query);
        while (rs.next()) {
    ...
    private void loadNoNode(Statement st, Integer BID1, Integer BID2) throws SQLException {
        // benchmark 1
        String query = "SELECT p.name FROM PROBLEMS p, RESOLUTIONS r WHERE bid = " + BID1 + " AND r.nb_nodes = 0 AND r.pid=p.pid";
        ResultSet rs = st.executeQuery(query);
        while (rs.next()) {
            prob_name[0][NO_NODE].add(rs.getString("name"));
        }
        // benchmark 2
        query = "SELECT p.name FROM PROBLEMS p, RESOLUTIONS r WHERE bid = " + BID2 + " AND r.nb_nodes = 0 AND r.pid=p.pid";
        rs = st.executeQuery(query);
        while (rs.next()) {
            prob_name[1][NO_NODE].add(rs.getString("name"));
        }
    }
